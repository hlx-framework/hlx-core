 ...
 * Used for a trampoline's own tail-jump into a fixed handler (log.c). Clobbers
 * RAX - fine here since `at` is always reached as the START of a fresh call,
 * with nothing live yet to clobber. Hook installation itself (patching.c) goes
 * through MinHook now, which builds its own relocated, register-safe
 * trampolines - this stub is no longer used for that. */
